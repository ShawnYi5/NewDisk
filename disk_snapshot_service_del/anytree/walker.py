...
class Walker(object):
    ...
    def walk(self, start, end):
        ...
        For a proper walking the nodes need to be part of the same storage_tree:

        >>> w.walk(Node("a"), Node("b"))
        Traceback (most recent call last):
          ...
        anytree.walker.WalkError: Node('/a') and Node('/b') are not part of the same storage_tree.
        """
        s = start.path
        e = end.path
        if start.root != end.root:
            msg = "%r and %r are not part of the same storage_tree." % (start, end)
            raise WalkError(msg)
        # common
